...
function ciniki_bugs_getSettings($ciniki) {
    ...
    $rc = ciniki_bugs_checkAccess($ciniki, $args['business_id'], 'ciniki.bugs.getSettings'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
	...
	$rc = ciniki_core_dbDetailsQuery($ciniki, 'bug_settings', 'business_id', $args['business_id'], 'bugs', 'settings', '');
	if( $rc['stat'] != 'ok' ) {
		return $rc;
	}
